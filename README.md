...
### Назначение
Измерять температуру воды (NTC термистор), управлять нагревом через реле 5V с коммутацией 220V, индицировать состояние RGB LED и 5 LED индикаторами, подавать звуковые сигналы через буззер.

### Платформа
ATtiny44A (SOP-14), тактовая частота 8 MHz (внутренний RC-генератор)
...
- **PB3 (pin 4)** → RESET (для ISP, внешний pull-up 10kΩ к VCC)
...
- **BUZZER** → **PA0 (pin 13)**, через MMBT2222A NPN, **ПРЯМАЯ ЛОГИКА**
...
| Режим | Температура | LED    | Цвет RGB                          |
|-------|-------------|--------|-----------------------------------|
| 1     | 40°C        | LED1 ● | 🔵 Синий (оттенок 180)           |
| 2     | 50°C        | LED2 ● | 🟢 Зелёный (оттенок 96)          |
| 3     | 60°C        | LED3 ● | 🟡 Желто-зелёный (оттенок 64)    |
| 4     | 80°C        | LED4 ● | 🟠 Желтый (оттенок 32)           |
| 5     | 90°C        |LED5 ●● | 🔴 Оранжево-красный (оттенок 16) |

### СПЕЦИАЛЬНЫЕ РЕЖИМЫ КИПЯЧЕНИЯ:
- **Полное кипячение** (по умолчанию): 20 минут максимум
- **Частичное кипячение** (partial_boil): 5 минут
...
- **Сброс:** `wdt_reset()` каждые 5мс в главном цикле
...
- **Обнаружение кипения:** при скорости нагрева < 1°C/сек на температуре >94°C
- **Защита от сухого кипячения:** контроль скорости роста температуры
- **Индикация объёма воды:** через скорость пульсации RGB LED
...
**Звуковые сигналы:**
1. **Подтверждение нажатия:** 30 мс писк
2. **Режим ALARMING:** 30-секундный сигнал
   - 0-10 сек → звук
   - 10-20 сек → тишина
   - 20-30 сек → звук
3. **Режим ERROR:** тревожный прерывистый сигнал 250 мс (синхронно с миганием красного LED)
4. **Завершение нагрева:** мелодичный тон (частота зависит от пресета)
...
- **Уровни удержания:** 0 (не нажата), 1 (>200мс), 2 (>3сек)
...
- LED индикаторы выключены
- **Скорость пульсации зависит от объёма воды**
  - Быстрая (15мс) → мало воды
  - Средняя (22мс) → средний объём
  - Медленная (30мс) → много воды

### BOILING:
- Реле включено (нагрев)
- RGB LED выключен (или индикация partial_boil при удержании BTN1)
- LED индикаторы выключены
- **Автоматическое определение кипения** по скорости нагрева
- partial_boil влияет на максимальное время (5 мин vs 20 мин)

### ALARMING:
- Реле выключено
- RGB LED пульсирует
- Buzzer: 30-секундная последовательность (10с вкл, 10с пауза, 10с вкл)
- **Мелодичный тон** в конце (частота зависит от пресета)
...
- LED_B (PA6 используется как MOSI)
- Или используйте джамперы для отключения R2, R4

### КАЛИБРОВКА NTC:

1. Измерить реальную температуру воды термометром
2. Записать значения в код:
```c
...
### 2. RAM (SRAM): 256 байт
- Глобальные переменные: ~50 байт
- Стек: ~20 байт
- Запас: достаточно для текущей логики

### 3. EEPROM: 256 байт
- Адрес 0x00: preset_idx (1 байт)
- Свободно: 255 байтов (для будущих функций)

### 4. Ограничения ISP программирования:
- Конфликт пинов с RGB LED (LED_R, LED_B)
...
```
main.c                          - Исходный код (версия 4.2)
Kettle_Controller_Netlist.txt   - Список соединений (v4.2)
Checklist.txt                   - Чек-лист настроек (v4.2)
kettle_schematic.html           - Интерактивная схема (v4.2)
```
...
**Автоматический расчёт через контрольные точки:**
```c
...
## НОВЫЕ ФУНКЦИИ В v4.2

### 1. Автоматическое определение кипения
Контроль скорости нагрева при температуре >94°C:
```c
uint16_t rate = (uint32_t)dT * 1000 / dt;
if (rate < 100) { // < 1°C/сек → кипение
    boil_detected = 1;
    enter_state(ALARMING);
}
```

### 2. Индикация объёма воды
Через скорость пульсации RGB LED:
```c
if (time_to_60C < 20000) breath_delay = 15;   // мало воды
else if (time_to_60C < 40000) breath_delay = 22; // среднее
else breath_delay = 30;                        // много воды
```

### 3. Мелодичные звуковые сигналы
Функция `play_tone()` для проигрывания тонов:
```c
play_tone(freq_hz, duration_ms);
// Частота зависит от пресета (600-1800 Гц)
```
